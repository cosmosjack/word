      ...
      <p><?php
         ...
         if ($is_pending_activation) {
            ...
            echo $fs->apply_filters('pending_activation_message', sprintf(
                    ...
                    fs_text('pending-activation-message', $slug), $first_name, '<b>' . $fs->get_plugin_name() . '</b>', '<b>' . $current_user->user_email . '</b>', fs_text('complete-the-install', $slug)
            ));
         } else if ($require_license_key) {
            $button_label = 'agree-activate-license';

            echo $fs->apply_filters('connect-message_on-premium', sprintf(fs_text('hey-x', $slug), $first_name) . '<br>' .
                sprintf(fs_text('thanks-for-purchasing', $slug), '<b>' . $fs->get_plugin_name() . '</b>'), $first_name, $fs->get_plugin_name()
            );
         } else {
            ...
            echo $fs->apply_filters($filter, sprintf(fs_text('hey-x', $slug), $first_name) . '<br>' .
                sprintf(
                    fs_text($default_optin_message, $slug), '<b>' . $fs->get_plugin_name() . '</b>', '<b>' . $current_user->user_login . '</b>', '<a href="' . $site_url . '" target="_blank">' . $site_url . '</a>', $freemius_link
                ), $first_name, $fs->get_plugin_name(), $current_user->user_login, '<a href="' . $site_url . '" target="_blank">' . $site_url . '</a>', $freemius_link
            );
         }
         ?></p>
...
//			),
   ...
   if (!empty($permissions)) :
      ...
      <div class="fs-permissions">
         ...
         <ul><?php foreach ($permissions as $id => $permission) : ?>
