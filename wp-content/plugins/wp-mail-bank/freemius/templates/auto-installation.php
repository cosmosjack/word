...
$plugin_title = sprintf(
    '<strong>%s</strong>', esc_html($title)
);

$sec_countdown = 30;
$countdown_html = sprintf(
    esc_js(fs_text('x-sec', $slug)), sprintf('<span class="fs-countdown">%s</span>', $sec_countdown)
);
...
$install_url = $fs->_get_sync_license_url(
    $plugin_id, true, array('auto_install' => 'true')
);
...
$credentials = request_filesystem_credentials(
    esc_url_raw($install_url), $method, false, WP_PLUGIN_DIR, array()
);
...
<div class="fs-modal fs-modal-auto-install">
   <div class="fs-modal-dialog">
      ...
      <div class="fs-modal-body">
         ...
         <?php else : ?>
            <p class="fs-installation-notice"><?php
               echo sprintf(
                   fs_esc_html('installing-in-n', $slug), $plugin_title, sprintf(
                       '<a href="%s" target="_blank">%s</a>', 'https://freemius.com', 'freemius.com'
                   ), $countdown_html
               )
               ?></p>
         <?php endif ?>
