...
if ($fs->is_payments_sandbox()) {
   ...
   $context_params['sandbox'] = FS_Security::instance()->get_secure_token(
       $fs->get_plugin(), $timestamp, 'checkout'
   );
      ...
      (function ($) {
         $(function () {
            ...
            FS.PostMessage.receiveOnce('install', function (data) {
               ...
               $.form('<?php
echo fs_nonce_url($fs->_get_admin_page_url('account', array(
...
<?php
$install_data = array_merge($fs->get_opt_in_params(), array(
    'activation_url' => fs_nonce_url($fs->_get_admin_page_url('', array(
            'fs_action' => $slug . '_activate_new',
            'plugin_id' => $plugin_id,
        )), $slug . '_activate_new')
    ))
