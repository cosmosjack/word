                        ...
                        <?php if ($is_paying) : ?>
                           <li>
                              <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST">
                                 ...
                                 <a href="#"
                                    onclick="if (confirm('<?php fs_echo('deactivate-license-confirm', $slug) ?>'))
                                                this.parentNode.submit();
                                             return false;"><i
                           ...
                           if (!$license->is_lifetime() &&
                              ...
                              <li>
                                 <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST">
                                    ...
                                    <a href="#"
                                       onclick="if (confirm('<?php printf(fs_text('downgrade-x-confirm', $slug), $plan->title, human_time_diff(time(), strtotime($license->expiration))) ?> <?php
                                       if (!$license->is_block_features) {
                                          printf(fs_text('after-downgrade-non-blocking', $slug), $plan->title);
                                       } else {
                                          printf(fs_text('after-downgrade-blocking', $slug), $plan->title);
                                       }
                                       ?> <?php fs_echo('proceed-confirmation', $slug) ?>'))
                                                      this.parentNode.submit();
                                                   return false;"><i class="dashicons dashicons-download"></i> <?php fs_echo(( $fs->is_only_premium() ? 'cancel-subscription' : 'downgrade'), $slug) ?></a>
                                 </form>
                              </li>
                              <li>&nbsp;&bull;&nbsp;</li>
                           <?php endif ?>
                        ...
                        <?php elseif ($is_paid_trial) : ?>
                           <li>
                              <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST">
                                 ...
                                 <a href="#"
                                    onclick="if (confirm('<?php fs_echo('cancel-trial-confirm') ?>'))
                                                this.parentNode.submit();
                                             return false;"><i
                                       ...
                                       <form action="<?php echo $fs->_get_admin_page_url('account') ?>"
                                          ...
                                          <input type="submit" class="button button-primary"
                                                 ...
                                                 echo esc_attr(sprintf(
                                                         fs_text('activate-x-plan', $slug) . '%s', $premium_plan->title, ( $site->is_localhost() && $available_license->is_free_localhost ) ?
                           ...
                           <?php if ('plan' !== $p['id']) : ?>
                              <td class="fs-right">
                                 ...
                                 <?php if ('version' === $p['id']) : ?>
                                    ...
                                    <?php endif ?>
                                 <?php elseif (in_array($p['id'], array('license_key', 'site_secret_key'))) :
                                    ?>
                                 ...
                                 elseif (/* in_array($p['id'], array('site_secret_key', 'site_id', 'site_public_key')) || */
                                    ...
                                    ?>
                                    <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST"
                                          onsubmit="var val = prompt('<?php echo esc_attr(sprintf(fs_text('what-is-your-x', $slug), $p['title'])) ?>', '<?php echo $p['value'] ?>');
                                                         if (null == val || '' === val)
                                                            return false;
                                                         jQuery('input[name=fs_<?php echo $p['id'] ?>_<?php echo $slug ?>]').val(val);
                                                         return true;">
                              ...
                              foreach ($addons_to_show as $addon_id) :
                                 ...
                                 ?>>
                                    ...
                                    <?php if ($is_addon_connected) : ?>
                                       ...
                                       if ($is_addon_activated) {
                                          if ($is_paying) {
                                             $buttons[] = fs_ui_get_action_button(
                                                 $slug, 'account', 'deactivate_license', fs_text('deactivate-license', $slug), array('plugin_id' => $addon_id), false
                                             );

                                             $human_readable_license_expiration = human_time_diff(time(), strtotime($license->expiration));
                                             $downgrade_confirmation_message = sprintf(fs_text('downgrade-x-confirm', $slug), $plan->title, $human_readable_license_expiration);
                                             ...
                                             if (!$license->is_lifetime() && $is_active_subscription) {
                                                $buttons[] = fs_ui_get_action_button(
                                                    $slug, 'account', 'downgrade_account', fs_text('downgrade', $slug), array('plugin_id' => $addon_id), false, false, ( $downgrade_confirmation_message . ' ' . $after_downgrade_message), 'POST'
                                                );
                                             }
                                          } else if ($is_paid_trial) {
                                             $buttons[] = fs_ui_get_action_button(
                                                 $slug, 'account', 'cancel_trial', fs_text('cancel-trial', $slug), array('plugin_id' => $addon_id), false, 'dashicons dashicons-download', fs_text('cancel-trial-confirm', $slug), 'POST'
                                             );
                                          } else {
                                             ...
                                             if (is_object($premium_license)) {
                                                ...
                                                $buttons[] = fs_ui_get_action_button(
                                                    $slug, 'account', 'activate_license', sprintf(fs_text('activate-x-plan', $slug), $fs_addon->get_plan_title(), ( $site->is_localhost() && $premium_license->is_free_localhost ) ? '[localhost]' : ( 1 < $premium_license->left() ? $premium_license->left() . ' left' : '' )), array(
                                                    'plugin_id' => $addon_id,
                                          ...
                                          if (0 == count($buttons)) {
                                             ...
                                             $buttons[] = fs_ui_get_action_button(
                                                 $slug, 'account', $slug . '_sync_license', fs_text('sync-license', $slug), array('plugin_id' => $addon_id), false
                                             );
                                          }
                                          ...
                                          if ($fs->is_addon_installed($addon->slug)) {
                                             ...
                                             $buttons[] = sprintf(
                                                 '<a class="button button-primary edit" href="%s" title="%s">%s</a>', wp_nonce_url('plugins.php?action=activate&amp;plugin=' . $addon_file, 'activate-plugin_' . $addon_file), fs_esc_attr('activate-this-addon', $slug), fs_text('activate', $slug)
                                             );
                                          } else {
                                             if ($fs->is_allowed_to_install()) {
                                                $buttons[] = sprintf(
                                                    '<a class="button button-primary edit" href="%s">%s</a>', wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $addon->slug), 'install-plugin_' . $addon->slug), fs_text('install-now', $slug)
                                                );
                                             } else {
                                                $buttons[] = sprintf(
                                                    '<a target="_blank" class="button button-primary edit" href="%s">%s</a>', $fs->_get_latest_download_local_url($addon_id), fs_text('download-latest', $slug)
                                                );
                                             }
                                          }
                                       }
                                       ...
                                       if ($show_upgrade) {
                                          $buttons[] = sprintf('<a href="%s" class="thickbox button button-primary" aria-label="%s" data-title="%s"><i class="dashicons dashicons-cart"></i> %s</a>', esc_url(network_admin_url('plugin-install.php?tab=plugin-information&parent_plugin_id=' . $fs->get_id() . '&plugin=' . $addon->slug .
                                                      '&TB_iframe=true&width=600&height=550')), esc_attr(sprintf(fs_text('more-information-about-x', $slug), $addon->title)), esc_attr($addon->title), fs_text(( $fs_addon->has_free_plan() ? 'upgrade' : 'purchase'), $slug)
                                          );
                                       }
                                    ...
                                    <?php if (defined('WP_FS__DEV_MODE') && WP_FS__DEV_MODE) : ?>
                                       <td>
                                          ...
                                          if ($is_addon_activated) {
                                             fs_ui_action_button(
                                                 $slug, 'account', 'delete_account', fs_text('delete', $slug), array('plugin_id' => $addon_id), false
                                             );
                                          }
                                          ?>
                                       </td>
