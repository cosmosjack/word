...
<div id="fs_addons" class="wrap">
   ...
   <div id="poststuff">
      <?php if (!$has_addons) : ?>
         <h3><?php
            printf(
                '%s... %s', fs_text('oops', $slug), fs_text('add-ons-missing', $slug)
            )
            ?></h3>
      <?php endif ?>
         ...
         <?php if ($has_addons) : ?>
            <?php foreach ($addons as $addon) : ?>
               ...
               <li class="fs-card fs-addon" data-slug="<?php echo $addon->slug ?>">
                  <?php
                  echo sprintf('<a href="%s" class="thickbox fs-overlay" aria-label="%s" data-title="%s"></a>', esc_url(network_admin_url('plugin-install.php?tab=plugin-information&parent_plugin_id=' . $fs->get_id() . '&plugin=' . $addon->slug .
                              '&TB_iframe=true&width=600&height=550')), esc_attr(sprintf(fs_text('more-information-about-x', $slug), $addon->title)), esc_attr($addon->title)
                  );
                  ?>
                  <?php
