...
//@include BASE_DATA_PATH.DS."api".DS."pdo".DS."debug.class.php";
include_once $_SERVER['DOCUMENT_ROOT']."/app/debug.class.php";
...
abstract class myDb {
		...
		protected function setLink() {
               // echo '这里到了DN底层类';
                $now_dnname = $_SESSION['dbname'] ? $_SESSION['dbname'] : DBNAME;
               // print_r($now_dnname);
			if(empty($this->dbconfig)) {
				//如果不是DSN，就设置和原配置文件一样	
				$this->charset = 'utf8';
				$this->dsn = defined('DSN') ? DSN : null;
				$this->host = HOST;
				$this->user = USER;
				$this->pass = PASS;
				$this->dbname = $now_dnname;
				$this->tabprefix = TABPREFIX;	
			
			} else {

				$filename = PROJECT_PATH.strtolower($this->dbconfig).'.db.php';
				...
				if(file_exists($filename)) {
					...
					$charset = self::getKeyValue('CHARSET', $config);
					$this->charset = empty($charset) ? CHARSET : $charset;
					$host =  self::getKeyValue('HOST', $config);
					$this->host = empty($host) ? HOST : $host;

					$user =  self::getKeyValue('USER', $config);
					$this->user = empty($user) ? USER : $user;
					$pass =  self::getKeyValue('PASS', $config);
					$this->pass = empty($pass) ? PASS : $pass;
					$dbname =  self::getKeyValue('DBNAME', $config);
					$this->dbname = empty($dbname) ? DBNAME : $dbname;
					$tabprefix =  self::getKeyValue('TABPREFIX', $config);
					$this->tabprefix = empty($tabprefix) ? TABPREFIX : $tabprefix;

				
				
					$key = 'DSN';
					$pre = preg_replace('/.*((\/\*|\/\/))\s*define\([\"\']'.$key.'[\"\'],\s*[\"\'](.+?)[\"\']\)\s*\;.*/is', '\1', $config);
					
					if($pre{0} == "/") {
						$this->dsn = null;
					
					} else {
						$this->dsn = self::getKeyValue('DSN', $config);
					
					} 


					if(empty($dsn) && defined('DSN')) {
						$this->dsn = DSN;
					}
					...
					echo "<font color='red'>配置文件".$filename."不存在!</font>";
		...
		static function getKeyValue($key, $content) {
		
			$arr = null;
			if(preg_match('/(\/\*|\/\/)?\s*define\([\"\']'.$key.'[\"\'],\s*[\"\'](.+?)[\"\']\)\s*\;/i',$content, $arr)) {
    ...
    /* 设置 sum 的 列名字 start */
    public function set_sum_cloumn($column){
        $this->sum = $column;
    }
    /* 设置 sum 的 列名字 end */
		/**
...
//        P($where);
        $sql="SELECT SUM({$this->sum}) as sum FROM {$this->tabName}{$where}";
//        P($sql);
        $result = $this->query($sql, __METHOD__,$data);
        return $result;
    }
...
//				$vali=Validate::check($array, "mod", $this);
		...
		private function comWhere($args){
			...
			if(count($args)==2 && is_string($args[0]) && is_array($args[1])) {
				...
				return array("where"=>$where.$args[0], "data"=>$args[1]);
			}
